 ...
 * an {@link  org.apache.ws.commons.schema.XmlSchema} using
 * the {@link XmlSchemaWalker}.
 *
 * <p>
 * Use this interface in conjunction with <code>XmlSchemaWalker</code>
 * to receive events as an {@link  org.apache.ws.commons.schema.XmlSchema}
 * is traversed.
 * </p>
 */
public interface XmlSchemaVisitor {
  ...
  /**
   * A notification that an {@link XmlSchemaElement} has been entered.
   * The element returned will be a true representation of the element
   * at that point in the schema: if the schema defines a reference,
   * the reference is followed and merged with its global definition.
   *
   * <p>
   * The first time this element is reached, all of its attributes will
   * be visited (if any).  Once the attributes have been visited,
   * {@link #onEndAttributes(XmlSchemaElement, XmlSchemaTypeInfo)} will
   * be called.  The only exception is when the element has no type
   * information, at which point the next call will be to
   ...
   * will be set to <code>true</code> and the attributes will not be revisited.
   ...
   * Provides the same information about the element as was provided
   * in {@link #onEnterElement(XmlSchemaElement, XmlSchemaTypeInfo, boolean)}
   * in the event it is easier to process on exit.
   *
   ...
   * (provided the element has a type defined).  This is a convenience
   * method to allow the visitor to be notified when no more attributes
   * are coming, and the walker will be traversing the element's children.
   *
   ...
   * is not called with the abstract type, as there is no information to
   * provide.
   * </p>
   * 
   * <p>
   ...
   * {@link #onEnterElement(XmlSchemaElement, XmlSchemaTypeInfo, boolean)} will
   * be called with the root element.
   * </p>
   *
  ...
  /**
   * Called when a wildcard attribute is visited.  If an element has a
   * wildcard element, this will be called after all other attributes
   * have been visited, and before the call to
   * {@link #onEndAttributes(XmlSchemaElement, XmlSchemaTypeInfo)}.
   *
   * @param element The owning element.
   * @param anyAttr The {@link XmlSchemaAnyAttribute} representing the wildcard
   *                attribute.
   */
